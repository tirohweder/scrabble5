...
public class AiPlayer extends Player {
        ...
        if (Data.getGameSession().getGameBoard().getPlayedTile(row, column) != null) {
          ...
          if (counterDown + counterUp > counterLeft + counterLeft) {
            wordList = wordGenerator(
                ...
                counterDown, counterUp, row, column, false);
          } else {
            wordList = wordGenerator(
                Data.getGameSession().getGameBoard().getPlayedTile(row, column).getLetter(),
                counterRight, counterLeft, row, column, true);
          }
  ...
  /*
   * public static void main(String[] args) { setLetterPoints(); String[] test =
   * wordGeneratorTest("A", 3, 4, 6, 6, true); for (String s : test) { System.out.println(s); } }
   */
